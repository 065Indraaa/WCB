...
function MyPositionBanner() {
  ...
  if (!connected) {
    return (
      <div style={{ padding: '1.25rem 1.5rem', borderRadius: 14, background: 'linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 100%)', border: '1.5px solid #BBF7D0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
        <div>
        ...
        <WalletMultiButtonDynamic style={{ background: 'linear-gradient(135deg, #15803D 0%, #22C55E 100%)', border: 'none', borderRadius: 10, fontSize: '0.85rem', fontWeight: 700, height: 36, padding: '0 1.25rem' }} />
      </div>
    );
  }

  if (loading) {
    return <div style={{ height: 80, borderRadius: 14, background: '#F1F5F0', marginBottom: '1.5rem' }} />;
  }

  return (
    <div style={{ padding: '1.25rem 1.5rem', borderRadius: 14, background: 'linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 100%)', border: '1.5px solid #BBF7D0', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
      {[
        ...
        <div key={s.label} style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '1.1rem', fontWeight: 900, color: '#15803D' }}>{s.value}</p>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-12">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '2rem' }}>
        <div>
          <p className="section-eyebrow mb-2">🏆 Top 10 Holders</p>
          ...
          <p className="text-lg max-w-2xl" style={{ color: '#64748B' }}>
            Top 10 $WCB holders on Solana. Click any row to view their Streamflow locks.
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <button onClick={refetch} style={{ padding: '0.5rem 0.875rem', borderRadius: 8, border: '1.5px solid #E2E8F0', background: '#ffffff', color: '#64748B', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer' }}>
            ↻ Refresh
          </button>
          <WalletButtonDynamic />
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          ...
          { label: 'Top Holders', value: leaderboard.length.toString(), icon: '👥' },
        ].map((s) => (
      ...
      <div className="card p-5 mb-6">
        <p style={{ fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#64748B', marginBottom: '0.75rem' }}>Tier Thresholds</p>
      ...
      {loading ? (
        ...
        <div className="card overflow-hidden">
          <table className="w-full" aria-label="$WCB Leaderboard">
            <thead>
          ...
          <div style={{ padding: '0.875rem 1.25rem', background: '#F8FAFC', borderTop: '1px solid #E2E8F0', textAlign: 'center' }}>
            <p style={{ fontSize: '0.72rem', color: '#94A3B8' }}>
              Top 10 $WCB holders · Token: <a href={`https://solscan.io/token/${WCB_MINT}`} target="_blank" rel="noopener noreferrer" style={{ color: '#15803D', textDecoration: 'none' }}>{WCB_MINT.slice(0, 8)}...{WCB_MINT.slice(-6)}</a> · Click any row to view on Streamflow
            </p>
          </div>
        </div>
      )}
      ...
      {/* CTA */}
      <div style={{ marginTop: '2rem', padding: '1.5rem', borderRadius: 14, background: 'linear-gradient(135deg, #15803D 0%, #22C55E 100%)', textAlign: 'center' }}>
        <p style={{ fontSize: '1rem', fontWeight: 800, color: '#ffffff', marginBottom: '0.5rem' }}>Want to climb the leaderboard?</p>
        <p style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.85)', marginBottom: '1.25rem' }}>Lock more $WCB for longer to earn more credits and secure your tier.</p>
        <a href="/lock" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem 1.75rem', borderRadius: 10, background: '#ffffff', color: '#15803D', fontWeight: 800, fontSize: '0.9rem', textDecoration: 'none' }}>
          🔒 Lock $WCB Now
        </a>
      </div>
    </div>
